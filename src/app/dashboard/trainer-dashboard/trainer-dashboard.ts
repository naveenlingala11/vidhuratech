...
export class TrainerDashboard implements OnInit {
      ...
      .subscribe({
        next: (res: any) => {
          ...
          this.upcomingSessions =
            data.sections?.upcomingSessions || [];
          this.studentActivities =
            data.sections?.studentActivities || [];
          this.loading = false;
        ...
        error: () => {
          this.loading = false;
          this.showToast(
            'Trainer dashboard failed to load'
          );
        }
      });
  }
  ...
  review(submission: any) {
    const draft =
      this.reviewDraft[submission.id];
    this.trainerService
      .reviewSubmission(
        submission.id,
        draft
